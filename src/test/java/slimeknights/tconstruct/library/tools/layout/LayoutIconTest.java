...
class LayoutIconTest extends BaseMcTest {
  ...
  void item_jsonSerialize() {
    ...
    assertThat(GsonHelper.getString(json, "item")).isEqualTo(Objects.requireNonNull(Items.DIAMOND_PICKAXE.getRegistryName()).toString());
    assert original.getTag() != null;
    assertThat(GsonHelper.getString(json, "nbt")).isEqualTo(original.getTag().toString());
  }
  ...
  void pattern_jsonSerialize() {
    ...
    assertThat(GsonHelper.getString(json, "pattern")).isEqualTo(pattern.toString());
  }
